  ...
  <q-page class="backlog-page">
    ...
    <template v-else>
      <div v-if="filtered.length === 0" class="empty">
        Aucun ticket{{ filter === 'all' ? '' : ' dans cette catégorie' }}.
      </div>
      ...
      <div v-for="t in filtered" :key="t.id" class="tk">
        ...
        <div v-if="t.screenshots?.length" class="tk-shots">
          <a
            v-for="(u, i) in t.screenshots"
            :key="i"
            :href="u"
            target="_blank"
            rel="noopener"
            class="tk-shot"
            ><img :src="u" alt="capture"
          /></a>
        </div>
        <div class="tk-meta">
          {{ fmtDate(t.created_at)
          }}<template v-if="t.app_version"> · v{{ t.app_version }}</template
          ><template v-if="t.page"> · {{ t.page }}</template>
        </div>
        <div class="tk-actions">
          <button v-if="t.status !== 'in_progress'" class="act" @click="set(t, 'in_progress')">
            En cours
          </button>
          <button v-if="t.status !== 'done'" class="act done" @click="set(t, 'done')">
            Traité
          </button>
      ...
      <div class="nt-sheet">
        ...
        <div class="kinds">
          <button
            ...
            :key="k.value"
            class="kind"
            :class="{ on: newKind === k.value }"
            @click="newKind = k.value"
          >
            {{ k.label }}
          </button>
        </div>
