...
// Dimensions de l'étage : NON carrées et VARIÉES (seedées) → chaque étage a une forme
// différente (3..5 large × 3..4 haut), au lieu d'un carré rigide. Reste petit = lisible
// sur mobile ; croît un peu en profondeur. `rng` en 1er dans generateFloor → déterministe.
function gridFor(rng: () => number, floorIndex: number): { cols: number; rows: number } {
  const depthBoost = Math.min(1, Math.floor(floorIndex / 2)); // 0 tôt → +1 en profondeur
  const cols = 3 + depthBoost + (rng() < 0.5 ? 1 : 0); // 3..5
...
export function generateFloor(seed: number, floorIndex: number, floors: number): Floor {
  ...
  const { cols, rows } = gridFor(rng, floorIndex);
