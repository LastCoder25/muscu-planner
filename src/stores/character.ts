...
export const useCharacterStore = defineStore('character', () => {
  ...
  function normalizeRow(r: CharacterRow | null): CharacterRow | null {
    ...
    // que la fusion du Panthéon). `fetchMine` persiste aussitôt (`settleKennel`).
    if (r.base) {
  ...
  async function fetchMine() {
    ...
    const kennel =
      rawBase && Array.isArray(rawBase.defenses)
        ? retireKennel(rawBase)
        : { gold: 0, scrap: 0, base: null };
    row.value = normalizeRow(data ?? null);
    loaded.value = true;
    if (row.value && legacy.goldRefund > 0) await settlePantheon(uid, legacy.goldRefund);
    if (row.value && (kennel.gold > 0 || kennel.scrap > 0))
      await settleKennel(uid, kennel.gold, kennel.scrap);
  ...
  /** 🐾 Écrit le retrait du Chenil **AUSSITÔT** : l'enceinte sans lui, l'or et la ferraille
   *  rendus, dans la MÊME requête — même raison que `settlePantheon` (tant que la base
   *  porte encore le Chenil, chaque chargement recalculerait le remboursement). On en
   *  profite pour écrire le vivier sans compagnons. Et on le DIT. */
  async function settleKennel(userId: string, gold: number, scrap: number) {
    const cur = row.value;
    if (!cur) return;
    try {
      ...
      return; // hors ligne : on retentera au prochain chargement.
    }
    const bits = [
      gold > 0 ? `${gold.toLocaleString('fr-FR')} 🪙` : '',
      scrap > 0 ? `${scrap} 🔩` : '',
    ]
      .filter(Boolean)
      .join(' et ');
    useGameFx().celebrate({
      ...
      subtitle: `Familiers et talents restent au héros — ${bits} rendus`,
      rarity: 'epic',
    });
  }
