...
- **Changer d'exo** → sheet 2 onglets : _Suggestions_ (via `alternatives` + même `muscle_primary`) et _Séances passées_ (requête `session_logs`, dernière charge + note). Sélection → remplace l'exo, **conserve la charge**, renseigne `swapped_from`.
