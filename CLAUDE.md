...
src/lib/
  ...
  importSession.ts parseImportedSession(raw, library) — import IA tolérant : strict (coach) → JSON « lâche » → **texte libre** (`parseWorkoutText` : format ChatGPT « 40 kg ×8 — 1 min », en-têtes emoji, back-off, notes 👉) ; enrichissement biblio (match flou nom), gainage→secondes, top set comme charge
