...
- **Stats** : `src/lib/volume.ts` (séries/groupe : `plannedSetsByMuscle`, `setsByMuscleFromLog`, `muscleColor`) ; affiché sur SessionDetail (prévu) + Bilan (prévu/réalisé). `StatsPage` (`/stats`, menu ⋮) : KPIs, séries par groupe (barres), exos par fréquence avec charge moy/médiane/max → lien vers la fiche (courbe 1RM). `LoggedExercise.muscle_primary` stocké au log.
