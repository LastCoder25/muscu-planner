  ...
  <q-page class="free-page">
      ...
      <div class="top-mid">
        ...
        <div class="top-sub">
          {{ clock
            ...
            · {{ run.exercises.length }} exos · vol {{ totalVolume }} kg</template
          >
        </div>
      </div>
      <q-btn
      ...
      <div ref="scrollEl" class="scroll">
        <div v-if="ex" class="exo">
            ...
            <div class="ring-wrap">
              <svg width="170" height="170" viewBox="0 0 184 184">
                ...
                <circle
                  ...
                  :stroke-dashoffset="515 * (1 - ringFrac)"
                />
              </svg>
              <div class="ring-num">
          ...
          <div
            ...
            :key="s.uid ?? i"
            class="set"
            :class="{ done: s.done, cur: i === curSetIndex }"
          >
            ...
            <div>
              ...
              <div class="val-line">
                <button
                  v-if="i === curSetIndex || i === editIdx"
                  class="stepper"
                  @click="adj(s, 'load_kg', -2.5)"
                >
                  −
                </button>
                <input
                  v-if="i === curSetIndex || i === editIdx"
                  v-model.number="s.load_kg"
                  type="number"
                  inputmode="decimal"
                  min="0"
                  step="0.5"
            ...
            <div>
              ...
              <button
                v-else-if="s.done"
                class="dpill editable"
                :class="'d' + (s.difficulty || 2)"
              ...
              <button
                v-else-if="i === curSetIndex && ex.sets.length > 1"
                class="rm"
                aria-label="Retirer la série"
                @click="live.removeSet(i)"
              >
                ✕
              </button>
            </div>
            <div v-if="i === editIdx" class="edit-diff">
              ...
              <button
                ...
                :key="d.n"
                class="ediff"
                :class="['d' + d.n, { sel: s.difficulty === d.n }]"
                @click="
                  s.difficulty = d.n;
                  live.persist();
                "
          ...
          <div v-if="rateTarget" class="comment">
            <div class="comment-head">
              <span>Ressenti de la série faite</span><em>optionnel</em>
            </div>
            <div class="diff-row">
              <button
                v-for="d in DIFFS"
                :key="d.n"
                class="diff-btn"
                :class="['d' + d.n, { sel: rateTarget.difficulty === d.n }]"
                @click="
                  rateTarget.difficulty = d.n;
                  live.persist();
                "
              >
                <b>{{ d.n }}</b
    ...
    <q-dialog v-model="pickerOpen" position="bottom">
      <div class="sheet">
        ...
        <div class="mus-filter">
          <button class="mchip" :class="{ on: muscleFilter === '' }" @click="muscleFilter = ''">
            Tous
          </button>
          <button
            ...
            :key="m"
            class="mchip"
            :class="{ on: muscleFilter === m }"
            @click="muscleFilter = m"
          >
            {{ m }}
          </button>
        </div>
        <div v-if="picked.length" class="picked-note">
          {{ picked.length }} exo{{ picked.length > 1 ? 's' : '' }} ajouté{{
            picked.length > 1 ? 's' : ''
          }}
        </div>
        ...
        <div v-else-if="pickerView === 'tiles'" class="tiles">
          <button
            ...
            :key="e.id"
            class="tile"
            :style="{ borderTopColor: exColor(e) }"
            @click="pick(e)"
          >
            <img
              v-if="exerciseImage(e.id)"
              :src="exerciseImage(e.id)"
              :alt="e.name"
              class="tile-img"
              loading="lazy"
            />
            <span
              v-else
              class="tile-badge"
              :style="{ background: exColor(e) + '22', color: exColor(e) }"
              ><q-icon :name="exIcon(e)" size="26px"
            /></span>
            <div class="tile-name">{{ e.name }}</div>
            <div v-if="e.muscle_primary" class="tile-mus" :style="{ color: exColor(e) }">
              {{ e.muscle_primary }}
            </div>
            <q-icon
              name="info_outline"
        ...
        <template v-else>
          <button v-for="e in filteredLib" :key="e.id" class="alt" @click="pick(e)">
            <span class="alt-badge" :style="{ background: exColor(e) + '22', color: exColor(e) }"
              ><q-icon :name="exIcon(e)" size="20px"
            /></span>
            <div class="alt-main">
              <div class="alt-name">{{ e.name }}</div>
              <div class="alt-meta">
                {{ e.muscle_primary }}<template v-if="e.equipment"> · {{ e.equipment }}</template>
              </div>
            </div>
            <q-icon
              name="info_outline"
        ...
        <button v-if="picked.length" class="cta q-mt-sm" @click="pickerOpen = false">
          Terminé ({{ picked.length }})
        </button>
      </div>
    </q-dialog>
    ...
    <q-dialog v-model="quitOpen" position="bottom">
      <div class="sheet">
        ...
        <button class="cta ghost q-mb-sm" @click="pauseSession">
          Mettre en pause<small> (reprendre plus tard)</small>
        </button>
        <button v-if="canFinish" class="cta q-mb-sm" @click="finishFromQuit">
...
const totalVolume = computed(() =>
  run.value
    ? run.value.exercises.reduce(
        (a, e) => a + e.sets.filter((s) => s.done).reduce((b, s) => b + s.load_kg * s.reps, 0),
        0,
      )
    : 0,
);
...
const pickerView = ref<'list' | 'tiles'>(
  localStorage.getItem(VIEW_KEY) === 'tiles' ? 'tiles' : 'list',
);
...
const filteredLib = computed(() => {
  ...
  if (n)
    base = base.filter(
      (e) => e.name.toLowerCase().includes(n) || (e.muscle_primary ?? '').toLowerCase().includes(n),
    );
