...
- **Challenges** : défis façon YouTube (100 pompes/jour…). Tables `challenges` + `achievements` (migr. 0012, RLS own) ; exos « challenge only » poids du corps/cardio (migr. 0013, `exercises.challenge_only` exclu du générateur). Logique pure `src/lib/challenges.ts` : formats **fixed/progressive/pyramid/pyramid_progressive/wave/cumulative** (`computeDailyTargets`), progressif **basé sur le MAX** (`progressiveApply` : J1=coef×MAX, +3–15 %/jour), `suggestConfig` (niveau+exo), `challengeStats` (streak/%/total), `evaluateAchievements`. Catalogues front `src/data/challengeFormats.ts` (+ `CHALLENGE_SUGGESTIONS`) et `achievements.ts`. Store `src/stores/challenges.ts`. Écrans : `ChallengesPage` (`/challenges`, onglets En cours/Terminés/Exercices/Succès), `ChallengeNewPage` (`/challenges/new` : exo favoris/suggestions → format → difficulté/durée/repos/rappel → aperçu), `ChallengeDetailPage` (`/challenges/:id` : calendrier + **exécution du jour** chrono pausable + accumulateur/boutons perso « nombres favoris » + saisie directe, validation auto). Multi-défis en parallèle. Rappel in-app `useChallengeReminder` (monté App.vue). Accès menu ⋮.
