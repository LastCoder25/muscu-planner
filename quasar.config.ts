...
export default defineConfig((/* ctx */) => {
  return {
    ...
    boot: ['qpage', 'theme', 'auth', 'pwa', 'notifyTap'],
    ...
    build: {
      ...
      // Charge .env / .env.local et expose au code client les variables
      // préfixées QCLI_ (défaut Quasar) et SUPABASE_ via import.meta.env.*.
      // La clé anon Supabase est PUBLIQUE par design (sécurité = RLS Postgres).
      env: {
        clientPrefix: ['QCLI_', 'SUPABASE_'],
      },
      ...
      // NB: vite-plugin-checker retiré — il spawne vue-tsc/eslint en
      // sous-processus via leurs shims .cmd, bloqués par AppLocker (poste
      // Orange). Type-check et lint restent dispo via `npm run typecheck`
      // et `npm run lint` (routés par node).
      vitePlugins: [],
    },
    ...
    framework: {
      config: {
        dark: true, // mode sombre forcé (design « panneau de contrôle »)
      },
