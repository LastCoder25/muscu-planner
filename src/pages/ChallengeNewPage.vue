<template>
  <q-page class="cn-page">
    ...
    <div class="scroll">
      ...
      <q-input v-model="search" filled dense placeholder="Rechercher un exercice…" class="q-mb-sm" clearable />
      <div v-if="loadingLib" class="row flex-center q-pa-md"><q-spinner color="primary" /></div>
      <div v-else class="ex-list">
        <button
          v-for="e in filteredLib" :key="e.id"
          class="ex-row" :class="{ sel: exercise?.id === e.id }"
          @click="pickExercise(e)"
        >
          <q-icon v-if="favSet.has(e.id)" name="star" size="16px" color="primary" class="fav" />
          <q-icon v-else-if="sugIndex.has(e.id)" name="local_fire_department" size="15px" color="primary" class="fav" />
          <div class="ex-main">
            <div class="ex-name">{{ e.name }}</div>
            <div class="ex-meta">{{ e.muscle_primary }} · {{ e.unit === 'time' ? 'temps' : 'reps' }}</div>
          </div>
      ...
      <template v-if="exercise">
        ...
        <div class="fmt-grid">
          <button
            v-for="f in CHALLENGE_FORMATS" :key="f.id"
            class="fmt" :class="{ sel: format === f.id }" @click="selectFormat(f.id)"
          >
        ...
        <div class="dur-grid q-mb-md">
          <button v-for="d in DURATIONS" :key="d" class="choice" :class="{ active: durationDays === d }" @click="setDuration(d)">
            {{ d === 100 ? '100 j' : d === 30 ? '1 mois' : d === 21 ? '3 sem' : d === 14 ? '2 sem' : '1 sem' }}
          </button>
        </div>
        ...
        <div class="cfg-grid q-mb-md">
          <template v-for="field in fields" :key="field">
            <div class="cfg-cell">
              ...
              <q-input
                :model-value="cfgDisplay(field)" type="number" filled dense
        ...
        <div class="days q-mb-md">
          <button v-for="w in WEEKDAYS" :key="w.value" class="choice small" :class="{ active: restDays.includes(w.value) }" @click="toggleRest(w.value)">{{ w.label }}</button>
        </div>
        ...
        <div class="row items-center q-mb-md" style="gap: 10px">
          ...
          <q-input v-if="reminderOn" v-model="reminderTime" type="time" filled dense style="max-width: 130px" />
        </div>
        ...
        <div class="preview">
          ...
          <div class="prev-bars">
            <div v-for="(t, i) in previewTargets" :key="i" class="prev-bar" :style="{ height: barH(t) + '%' }" :title="`J${i + 1} : ${t}`" />
          </div>
          <div class="prev-sub">
            <template v-if="format === 'cumulative'">Total : <b>{{ config.total }}</b> {{ unitLabel }} en {{ durationDays }} j</template>
            <template v-else>{{ activeDaysCount }} jours actifs · total ~<b>{{ totalPlanned }}</b> {{ unitLabel }}</template>
          </div>
        </div>
      </template>
    </div>
    ...
    <div v-if="exercise" class="cta-wrap">
      <button class="cta" :disabled="creating" @click="createChallenge">{{ creating ? 'Création…' : 'Lancer le challenge' }}</button>
    </div>
  </q-page>
</template>
...
import { CHALLENGE_FORMATS, DURATIONS, CHALLENGE_SUGGESTIONS, formatOption } from '@/data/challengeFormats';
import { computeDailyTargets, suggestConfig, progressiveApply, type ChallengeFormat, type ChallengeConfig } from '@/lib/challenges';
...
function fieldLabel(f: string) {
  return { start: 'Départ', increment: '+/jour', peak: 'Pic', cycle_days: 'Cycle (j)', deload_pct: 'Décharge %', total: 'Total', max: 'Ta perf max', start_coef: 'Départ (×MAX)', inc_pct: '+ %/jour' }[f] ?? f;
}
...
function setField(f: string, v: unknown) {
  const n = Number(v) || 0;
  if (f === 'deload_pct') config.value = { ...config.value, deload_pct: Math.min(1, Math.max(0, n / 100)) };
  else config.value = { ...config.value, [f]: n };
  if (format.value === 'progressive' && (f === 'max' || f === 'start_coef' || f === 'inc_pct')) applyProgressive();
}
function reset() {
  if (!exercise.value) return;
  config.value = suggestConfig(unit.value, level.value, format.value, durationDays.value, exercise.value.id);
...
const previewTargets = computed(() =>
  computeDailyTargets(format.value, { ...config.value, rest_weekdays: restDays.value }, durationDays.value, startDate),
);
const maxTarget = computed(() => Math.max(1, ...previewTargets.value));
function barH(t: number) { return Math.round((t / maxTarget.value) * 100); }
...
.cn-page { background: var(--bg); min-height: 100vh; display: flex; flex-direction: column; }
...
.sec-h { font-family: var(--font-display); font-size: 13px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: var(--dim); margin: 22px 2px 10px; }
.lbl { font-size: 12px; color: var(--dim); margin-bottom: 8px; }

.ex-list { max-height: 260px; overflow-y: auto; display: flex; flex-direction: column; gap: 6px; }
.ex-row { display: flex; align-items: center; gap: 10px; width: 100%; text-align: left; padding: 11px 12px; border-radius: 12px; background: var(--surface); border: 1px solid var(--line-soft); cursor: pointer; &.sel { border-color: var(--accent); background: var(--surface-2); } }
.ex-main { flex: 1; min-width: 0; }
.ex-name { font-weight: 600; font-size: 14.5px; color: var(--text); }
.ex-meta { font-size: 11.5px; color: var(--dim); text-transform: capitalize; }
.fav { flex: none; }

.fmt-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
.fmt { text-align: left; background: var(--surface); border: 1.5px solid var(--line); border-radius: 14px; padding: 12px; cursor: pointer; color: var(--text); &.sel { border-color: var(--accent); background: var(--surface-2); } }
.fmt-name { font-weight: 600; font-size: 14px; margin-top: 6px; }
.fmt-desc { font-size: 11px; color: var(--dim); margin-top: 2px; line-height: 1.25; }

.dur-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; }
.cfg-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
.cfg-lbl { font-size: 11px; color: var(--dim); margin-bottom: 3px; }
.days { display: grid; grid-template-columns: repeat(7, 1fr); gap: 6px; }
.choice { min-height: 44px; background: var(--surface); border: 1.5px solid var(--line); border-radius: 12px; color: var(--text); font-size: 14px; cursor: pointer; &.active { border-color: var(--accent); background: var(--surface-2); } &.small { min-height: 40px; font-size: 12px; padding: 0 2px; } }

.preview { background: var(--surface); border: 1px solid var(--line-soft); border-radius: 14px; padding: 12px; margin-top: 8px; }
.prev-h { font-size: 11px; color: var(--dim); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }
.prev-bars { display: flex; align-items: flex-end; gap: 2px; height: 70px; }
.prev-bar { flex: 1; min-width: 2px; background: var(--accent); border-radius: 2px 2px 0 0; min-height: 2px; opacity: 0.85; }
.prev-sub { font-size: 12px; color: var(--dim); margin-top: 8px; b { color: var(--text); } }

.cta-wrap { position: fixed; left: 0; right: 0; bottom: 0; max-width: 600px; margin: 0 auto; padding: 14px 16px 24px; background: linear-gradient(180deg, transparent, var(--bg) 30%); }
.cta { width: 100%; height: 56px; border: none; border-radius: 16px; background: var(--accent); color: var(--accent-ink); font-family: var(--font-display); font-weight: 700; font-size: 17px; text-transform: uppercase; letter-spacing: 0.5px; cursor: pointer; &:disabled { opacity: 0.5; } }
</style>
