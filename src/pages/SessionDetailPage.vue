  ...
  <q-page class="detail-page">
    <header class="top">
      ...
      <div class="top-mid">
        ...
        <div class="top-sub" v-if="session">
          {{ session.exercises.length }} exercices · ~{{ estimateDurationMin(session) }} min
        </div>
      </div>
    ...
    <template v-else-if="session">
      ...
      <div class="scroll">
        <div class="vol-groups">
          <span
            v-for="g in muscleVolume"
            :key="g.muscle"
            class="vol-chip"
            :style="{ borderColor: muscleColor(g.muscle) }"
          >
            <span class="vol-dot" :style="{ background: muscleColor(g.muscle) }" />{{ g.muscle }} ·
            <b>{{ g.sets }}</b>
          </span>
        </div>
          ...
          <div class="ex-top">
            ...
            <div class="ex-main">
              ...
              <div class="ex-muscles">
                {{ ex.muscle_primary
                }}<template v-if="ex.muscle_secondary?.length">
                  · <span class="dim">{{ ex.muscle_secondary.join(', ') }}</span></template
                >
              </div>
            </div>
          </div>
          <div class="ex-sub">
            {{ restLabel(ex) }}<template v-if="ex.unilateral"> · par côté</template
            ><template v-if="ex.equipment"> · {{ ex.equipment }}</template>
          </div>
...
const session = computed<Session | null>(
  () => sessionsStore.list.find((s) => s.id === id)?.payload ?? null,
);
...
function seriesOf(ex: PlannedExercise): { top: string; bottom: string }[] {
  if (ex.prescription?.length) {
    return ex.prescription.map((p) => ({
      top: p.load_kg ? `${p.load_kg}` : 'PdC',
      bottom: `× ${p.reps}`,
    }));
  }
  const t = ex.target;
  ...
  const bottom =
    t.unit === 'time' ? `${t.reps_min}–${t.reps_max}s` : `× ${t.reps_min}–${t.reps_max}`;
  const top =
    t.load === 'bodyweight'
      ? t.added_kg
        ? `+${t.added_kg}`
...
function restLabel(ex: { rest_seconds: number; prescription?: PrescribedSet[] }): string {
  const rests = (ex.prescription ?? [])
    .map((p) => p.rest_seconds)
