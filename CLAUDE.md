...
- **Backlog** : table `feedback` (migr. 0005 RLS own + 0007 read/update-all par tout connecté). Store `src/stores/feedback.ts` (submit/fetchMine + fetchAll/setStatus). UI : envoi + mes tickets dans `SettingsPage` ; **gestion** dans `BacklogPage` (`/backlog`, filtres + statuts). Hors-app : `node scripts/backlog.mjs`.
