...
describe('calibration du siège', () => {
  ...
  describe('🐾 LE CHENIL EST RETIRÉ, SON INVESTISSEMENT RENDU (v0.996)', () => {
    // Familiers et talents sont réservés au héros : le Chenil n'avait plus aucun effet.
    // ⚠️ Personne ne perd l'or ni la ferraille qu'il y a mis — la pose (750) et chaque cran,
    // aux prix RÉELS (`defenseUpgradeCost`/`defenseUpgradeScrap`), jamais un montant écrit.
    ...
    it('rend la pose et chaque cran, au prix réel', () => {
      for (const lvl of [1, 2, 17, 30]) {
        let or = 750;
        let fer = 0;
        for (let l = 1; l < lvl; l++) {
        ...
        const r = retireKennel(avec(lvl));
        expect(r.gold, `niveau ${lvl}`).toBe(or);
        expect(r.scrap, `niveau ${lvl}`).toBe(fer);
      }
    });
    it('retire le Chenil et garde le reste de l’enceinte', () => {
      const r = retireKennel(avec(9));
      expect(r.base.defenses).toEqual([{ typeId: 'wall', level: 12 }]);
    });
    it('⚠️ IDEMPOTENT : sans Chenil, rien n’est rendu et la base est la MÊME référence', () => {
      ...
      // Rejoué sur son propre résultat : plus rien à rendre (pas de double remboursement).
      const une = retireKennel(avec(5));
      expect(retireKennel(une.base).gold).toBe(0);
    });
  });
